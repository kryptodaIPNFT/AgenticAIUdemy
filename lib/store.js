...
function loadBundled(file, fallback) {
  try {
    ...
    if (file === 'thumbnail-manifest.json') return require('../data/thumbnail-manifest.json');
    if (file === 'logo-manifest.json') return require('../data/logo-manifest.json');
...
const SEEDS = {
  ...
  'thumbnail-manifest.json': loadBundled('thumbnail-manifest.json', []),
  'logo-manifest.json': loadBundled('logo-manifest.json', { file: 'westmont-hub.svg' })
};
...
async function getThumbnailManifest() {
  const bundled = SEEDS['thumbnail-manifest.json'];
  if (Array.isArray(bundled) && bundled.length) return bundled;
  return getJson('thumbnail-manifest.json', []);
}

async function getLogoManifest() {
  const bundled = SEEDS['logo-manifest.json'];
  if (bundled && typeof bundled === 'object' && bundled.file) return bundled;
  const fromStore = await getJson('logo-manifest.json', null);
  if (fromStore && typeof fromStore === 'object' && fromStore.file) return fromStore;
  return bundled && typeof bundled === 'object' ? bundled : { file: 'westmont-hub.svg' };
}
